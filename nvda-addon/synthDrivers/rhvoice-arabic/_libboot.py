...
Other NVDA add-ons put their OWN, often broken,
...
2. Putting our libs/ FIRST on sys.path and adding our numpy's DLL folder
   via os.add_dll_directory (Windows) so the C-extensions resolve.
3. Importing numpy and onnxruntime, then keeping our libs on the path so
   dependent imports also resolve.
...
def boot():
    ...
    try:
        ...
        # keep our libs reachable (append) but restore prior ordering
        sys.path[:] = saved_path
    ...
    if not ok:
        try:
            ...
            log.warning("RHVoice Arabic _libboot failed: %s" % last_error,
                        exc_info=True)
        except Exception:
            pass
    return ok
