/* Copyright (C) 2013, 2014, 2018  RHVOICE ARABIC <yakovleva.o.v@gmail.com> */
...
namespace
{
  ...
  jstring string_to_jstring(JNIEnv* env,const std::string& cppstr)
  {
    ...
    try {
      ...
      jchars.clear();
      auto it = cppstr.begin();
      while (it != cppstr.end()) {
        try {
          uint32_t cp = utf8::next(it, cppstr.end());
          if (cp > 0xFFFF) {
            jchars.push_back(static_cast<jchar>(0xD800 + ((cp - 0x10000) >> 10)));
            jchars.push_back(static_cast<jchar>(0xDC00 + ((cp - 0x10000) & 0x3FF)));
          } else {
            jchars.push_back(static_cast<jchar>(cp));
          }
        } catch (...) {
          if (it != cppstr.end()) ++it;
          jchars.push_back(static_cast<jchar>(' '));
        }
      }
    }
    if (jchars.empty()) {
      return check(env, env->NewStringUTF(""));
    }
  ...
  inline std::string jstring_to_string(JNIEnv* env,jstring jstr)
  {
    ...
    try {
      ...
      result.clear();
      const jchar* p = handle.str();
      const jchar* end = p + handle.length();
      while (p < end) {
        uint32_t cp = *p++;
  ...
  void speak_impl::operator()()
  {
    ...
    doc->speech_settings.punctuation.mode=RHVoice_punctuation_none;
    doc->set_flags(1);
    doc->set_owner(*this);
    doc->synthesize();
  }
  ...
  public:
    java_logger_wrapper(JNIEnv* env,jobject jimpl):
      ...
      Logger_log_method(get_method(env,check(env,env->GetObjectClass(jimpl)),"log","(Ljava/lang/String;Lorg/pbt/rh/ar/LogLevel;Ljava/lang/String;)V"))
    {
      env->GetJavaVM(&jvm);
      check(env);
    }
...
{
  TRY
    RHVoiceException_class=find_class(env,"org/pbt/rh/ar/RHVoiceException");
  LanguageInfo_class=find_class(env,"org/pbt/rh/ar/LanguageInfo");
  ...
  VoiceInfo_class=find_class(env,"org/pbt/rh/ar/VoiceInfo");
  ...
  VoiceInfo_setLanguage_method=get_method(env,VoiceInfo_class,"setLanguage","(Lorg/pbt/rh/ar/LanguageInfo;)V");
  VoiceInfo_setId_method=get_string_setter(env,VoiceInfo_class,"setId");
    SynthesisParameters_class=find_class(env,"org/pbt/rh/ar/SynthesisParameters");
  ...
  data_field=get_field(env,TTSEngine_class,"data","J");
    LogLevel_enum=find_class(env,"org/pbt/rh/ar/LogLevel");
    const char* sig="Lorg/pbt/rh/ar/LogLevel;";
