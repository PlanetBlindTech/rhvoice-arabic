...
MY_PACKAGE := RHVoice
MY_VERSION := 1.5.10
MY_COMMON_DEFINES := -DENABLE_PKG=1 -DBOOST_NO_CXX23_HDR_STDFLOAT=1
MY_CORE_DEFINES := -DPACKAGE=\"$(MY_PACKAGE)\" -DVERSION=\"$(MY_VERSION)\" -DDATA_PATH=\"\" -DCONFIG_PATH=\"\" -DENABLE_SONIC=0 -DHTS_EMBEDDED -DMAX_VOLUME=MAX_MAX_VOLUME  -DDEFAULT_PUNCTUATION_MODE=RHVoice_punctuation_none -DMAX_RATE=3
MY_ROOT_DIR := ../../../../../RHVoice
MY_SRC_DIR := $(MY_ROOT_DIR)/src
MY_INCLUDE_DIR := $(MY_SRC_DIR)/include
MY_BOOST_INCLUDE_DIR := $(MY_ROOT_DIR)/external/libs/boost/include
...
LOCAL_C_INCLUDES := $(LOCAL_PATH)/$(MY_INCLUDE_DIR) $(LOCAL_PATH)/$(MY_UTF8_DIR) $(LOCAL_PATH)/$(MY_RAPIDXML_DIR) $(LOCAL_PATH)/$(MY_HTS_ENGINE_DIR) $(LOCAL_PATH)/$(MY_BOOST_INCLUDE_DIR) $(LOCAL_PATH)/generated
